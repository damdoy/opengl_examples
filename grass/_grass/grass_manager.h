...
public:
   ...
   void init(Plane_sine *plane_sine){
      ...
      this->plane_sine = plane_sine; //used to position the grass blades
      ...
      _pid = load_shaders("grass_blades_vshader.glsl", "grass_blades_fshader.glsl");

      vao_grass = 0;

      //setting vao and vbo only one time for all grass blades
      {
         glGenVertexArrays(1, &vao_grass);
         glBindVertexArray(vao_grass);

         //index for the grass blade
         //  0
         //
         // 1.2
         // 3 4
         ...
         //texture coord definition, will allow to draw the grass
      ...
      for (size_t i = 0; i < nb_grass_elem_side; i++) {
         for (size_t j = 0; j < nb_grass_elem_side; j++) {
            ...
            mat_vector.push_back(ge->get_transf_0());
